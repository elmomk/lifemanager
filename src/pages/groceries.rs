...
pub fn Groceries() -> Element {
    ...
    use_effect(move || {
        let _ = refresh();
        spawn(async move {
            match checklist::list_checklist(ItemCategory::Grocery).await {
                Ok(loaded) => items.set(loaded),
                Err(e) => tracing::error!("Failed to load groceries: {e}"),
            }
        });
    });
    ...
    let add_item = move |text: String| {
        ...
        spawn(async move {
            if checklist::add_checklist(text, ItemCategory::Grocery, date).await.is_ok() {
    ...
    rsx! {
        SwipeItem {
            ...
            div { class: "flex items-center gap-3",
                ...
                if item.done {
                    span { class: "text-xs text-green-500 font-medium", "Got it" }
                }
            }
        }
    }
